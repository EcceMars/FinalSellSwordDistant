...
func tick(uid:int, behavior_component:BehaviorComponent = null)->Status:
	...
	if entry and entry.type == MT.WATER:
		behavior_component.blackboard.water_location = behavior_component.blackboard.mem_entry.position
		behavior_component.blackboard.where_to = behavior_component.blackboard.mem_entry.position
		return Status.DONE
	
	var best:MemoryComponent.Memory = memory.remember_closer(MT.WATER)
	if not best: return Status.ABORTED
	
	behavior_component.blackboard.water_location = best.position
